...
- HTTP request completion logging uses stable semantic convention attribute names such as `http.request.method`, `http.response.status_code`, `url.path`, `client.address`, and `user_agent.original`.
