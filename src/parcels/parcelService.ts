...
export function parseBBox(value: string): BBox {
  ...
  if (swLng >= neLng || swLat >= neLat) throw new Error('bbox southwest corner must be below and left of northeast corner');
...
export class ParcelService {
  ...
  async queryParcels(options: ParcelQueryOptions): Promise<{ features: ParcelFeature[]; limit: number; offset: number }> {
    const where: string[] = [];
    ...
    if (options.bbox) {
      ...
      where.push(`ST_Intersects(geom, ST_MakeEnvelope($${params.length - 3}, $${params.length - 2}, $${params.length - 1}, $${params.length}, 4326))`);
    }

    if (options.lng !== undefined || options.lat !== undefined || options.distanceMeters !== undefined) {
      if (options.lng === undefined || options.lat === undefined || options.distanceMeters === undefined) {
  ...
  async insertParcel(geometry: GeoJsonPolygon, properties: ParcelProperties): Promise<string> {
    const result = await this.pool.query(
      `INSERT INTO parcels (external_id, owner_id, properties, geom)
       ...
       RETURNING id::text`,
      [properties.externalId ?? null, properties.ownerId ?? null, properties.metadata ?? {}, JSON.stringify(geometry)],
    );
