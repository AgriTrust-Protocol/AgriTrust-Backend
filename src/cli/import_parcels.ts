...
async function main(): Promise<void> {
  ...
  if (!file.toLowerCase().endsWith('.geojson') && !file.toLowerCase().endsWith('.json')) {
    throw new Error(
      'GeoJSON import is supported directly. Convert Shapefiles to GeoJSON before import.',
    );
  }
  ...
  for (const [index, feature] of features.entries()) {
    try {
      if (feature.geometry?.type !== 'Polygon')
        throw new Error('Only Polygon parcel geometries are supported');
      await service.insertParcel(feature.geometry, {
        externalId: readProperty(feature.properties, [
          'external_id',
          'externalId',
          'parcel_id',
          'id',
