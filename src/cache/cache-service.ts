...
export class CacheService {
  ...
  async getJson<T>(key: string, options: CacheGetOptions = {}): Promise<T | null> {
    ...
    try {
      ...
      recordCacheOperation(
        'get',
        options.critical ? 'critical_error' : 'error',
        Date.now() - started,
      );
      return null;
    }
  ...
  async setJson<T>(key: string, value: T, options: CacheSetOptions = {}): Promise<boolean> {
    ...
    const ttlSeconds =
      options.ttlSeconds ??
      (options.critical ? this.config.criticalPathTtlSeconds : this.config.defaultTtlSeconds);
    const started = Date.now();
    try {
      const result = await this.withTimeout(
        this.client.set(this.scopedKey(key), JSON.stringify(value), 'EX', ttlSeconds),
      );
