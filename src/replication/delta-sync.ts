...
export type CRDTDelta =
  | {
      kind: 'lww';
      field: 'batchStatus' | 'escrowState';
      snapshot: ReturnType<LWWRegister<string>['toSnapshot']>;
      lsn: string;
      sourceRegion: string;
    }
  | {
      kind: 'orset';
      field: 'attestations';
      snapshot: ReturnType<ORSet<string>['toSnapshot']>;
      lsn: string;
      sourceRegion: string;
    }
  | {
      kind: 'pncounter';
      field: 'inventory';
      snapshot: ReturnType<PNCounter['toSnapshot']>;
      lsn: string;
      sourceRegion: string;
    };
...
export class DeltaSync {
  ...
  constructor(
    private readonly state: ApplicationCRDTState,
...
export function encodeDelta(delta: CRDTDelta): Buffer {
  return Buffer.from(JSON.stringify(delta));
}
export function decodeDelta(buffer: Buffer): CRDTDelta {
  return JSON.parse(buffer.toString('utf8')) as CRDTDelta;
}
