...
export class WalShipper {
  constructor(
    ...
    private readonly streamFactory: ReplicaStreamFactory,
  ...
  toDelta(change: WalChange): CRDTDelta {
    ...
    if (change.table === 'attestations')
      return {
        kind: 'orset',
        field: 'attestations',
        snapshot: {
          adds: { [String(change.value)]: [`${change.rowId}:${change.lsn}`] },
          removes: {},
        },
        lsn: change.lsn,
    ...
    if (change.table === 'inventory')
      return {
        kind: 'pncounter',
        field: 'inventory',
        snapshot: { increments: { [this.sourceRegion]: Number(change.value) }, decrements: {} },
        lsn: change.lsn,
        sourceRegion: this.sourceRegion,
      };
    return {
      kind: 'lww',
      field: change.column === 'escrow_state' ? 'escrowState' : 'batchStatus',
      snapshot: { value: String(change.value), timestamp },
      lsn: change.lsn,
      sourceRegion: this.sourceRegion,
    };
  }
  ...
  async ship(change: WalChange, replicas: RegionReplicaConfig[]): Promise<DeltaAck[]> {
    ...
    return Promise.all(
      replicas.map(async (replica) => {
        ...
        replicationBytesShippedTotal.inc(
          { source_region: this.sourceRegion, target_region: replica.id },
          payload.byteLength,
        );
        return ack;
      }),
