...
async function main(): Promise<void> {
  const direction = (process.env.MIGRATION_DIRECTION ?? process.argv[2] ?? 'up') as
    MigrationDirection | 'status' | 'plan';
  ...
  try {
    ...
    const result =
      direction === 'down' ? await manager.down(targetVersion) : await manager.up(targetVersion);
