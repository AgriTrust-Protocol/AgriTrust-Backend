  ...
  replicas: [
    {
      ...
      region: 'us-east-1',
      role: 'active',
      endpoint: process.env.REPLICATION_ACTIVE_ENDPOINT ?? 'grpc://active.replication.local:8443',
      maxBandwidthMbps: 50,
    },
    {
      id: 'aws-us-west-2',
      ...
      role: 'passive',
      endpoint:
        process.env.REPLICATION_PASSIVE_US_WEST_ENDPOINT ?? 'grpc://us-west.replication.local:8443',
      maxBandwidthMbps: 50,
    },
    {
      id: 'gcp-us-central1',
      ...
      role: 'passive',
      endpoint:
        process.env.REPLICATION_PASSIVE_GCP_CENTRAL_ENDPOINT ??
        'grpc://gcp-central.replication.local:8443',
      maxBandwidthMbps: 50,
    },
    {
      id: 'aws-eu-west-1',
      ...
      role: 'passive',
      endpoint:
        process.env.REPLICATION_PASSIVE_EU_WEST_ENDPOINT ?? 'grpc://eu-west.replication.local:8443',
      maxBandwidthMbps: 50,
    },
  ],
};
...
export function validateReplicationConfig(config = replicationConfig): void {
  ...
  if (active.length !== 1)
    throw new Error('replication topology requires exactly one active primary');
  if (passive.length > maxPassiveReplicas)
    throw new Error(`replication topology supports at most ${maxPassiveReplicas} passive replicas`);
  if (config.replicas.some((replica) => replica.maxBandwidthMbps > 50))
    throw new Error('replica link bandwidth must not exceed 50 Mbps');
}
