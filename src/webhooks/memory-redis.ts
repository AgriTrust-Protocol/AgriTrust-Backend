export class MemoryRedis {
  ...
  async zrangebyscore(
    ...
    let rows = [...(this.sortedSets.get(key)?.entries() ?? [])]
    ...
    if (limitIndex >= 0)
      rows = rows.slice(
        ...
        Number(args[limitIndex + 1]) + Number(args[limitIndex + 2]),
      );
    return rows.map(([m]) => m);
  }
  ...
  async lrem(key: string, _count: number, value: string): Promise<number> {
    ...
    return before - (this.lists.get(key)?.length ?? 0);
  }
  ...
  async set(
    ...
    const existing = this.strings.get(key);
    if (existing?.expiresAt && existing.expiresAt <= Date.now()) this.strings.delete(key);
