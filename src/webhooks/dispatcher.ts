...
import { recordWebhookAttempt, setWebhookQueueDepth } from './metrics';
import { signWebhookPayload, WEBHOOK_SIGNATURE_HEADER, WEBHOOK_TIMESTAMP_HEADER } from './signature';
...
export class WebhookDispatcher {
  ...
  async dispatch(input: Omit<WebhookDelivery, 'id' | 'idempotencyKey' | 'attempt' | 'createdAt' | 'nextRetryAt' | 'errors'> & Partial<Pick<WebhookDelivery, 'id' | 'idempotencyKey'>>): Promise<WebhookDelivery> {
    const now = Date.now();
    const delivery: WebhookDelivery = { id: input.id ?? randomUUID(), tenantId: input.tenantId, subscriptionId: input.subscriptionId, url: input.url, eventType: input.eventType, payload: input.payload, secret: input.secret, idempotencyKey: input.idempotencyKey ?? randomUUID(), attempt: 0, createdAt: now, nextRetryAt: now, errors: [] };
    await this.queue.enqueue(delivery);
    setWebhookQueueDepth(await this.queue.depth());
    return delivery;
  }
  ...
  async tick(now = Date.now()): Promise<void> {
    ...
    for (const delivery of due) {
      if ((this.activeByTenant.get(delivery.tenantId) ?? 0) >= this.config.tenantConcurrency) { await this.queue.enqueue({ ...delivery, nextRetryAt: now + this.config.workerPollIntervalMs }); setWebhookQueueDepth(await this.queue.depth()); continue; }
  ...
  private async attempt(delivery: WebhookDelivery): Promise<void> {
    ...
    try {
      ...
      const headers: Record<string, string> = { 'content-type': 'application/json', 'x-idempotency-key': current.idempotencyKey, 'x-webhook-id': current.id, 'x-event-type': current.eventType, [WEBHOOK_TIMESTAMP_HEADER]: String(timestamp) };
      if (current.secret) headers[WEBHOOK_SIGNATURE_HEADER] = signWebhookPayload(current.secret, body, timestamp);
      const response = await fetch(current.url, { method: 'POST', body, headers, signal: controller.signal });
      clearTimeout(timer);
      if (response.status >= 200 && response.status < 300) { await this.queue.remove(current.id); recordWebhookAttempt(current.tenantId, current.eventType, 'success'); setWebhookQueueDepth(await this.queue.depth()); return; }
      ...
      recordWebhookAttempt(current.tenantId, current.eventType, 'dead_letter');
    } catch (err) {
      ...
      if (failed.attempt >= this.config.maxRetries || Date.now() - failed.createdAt >= this.config.maxAttemptWindowMs) { await this.deadLetters.add(failed, 'max_retries_exhausted'); recordWebhookAttempt(current.tenantId, current.eventType, 'dead_letter'); return; }
      await this.queue.enqueue({ ...failed, nextRetryAt: Date.now() + retryDelayMs(failed.attempt, this.config.retryJitterRatio) });
      recordWebhookAttempt(current.tenantId, current.eventType, 'retry');
      setWebhookQueueDepth(await this.queue.depth());
    }
  }
}
