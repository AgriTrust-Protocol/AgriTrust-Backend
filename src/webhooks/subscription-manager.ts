...
export class SubscriptionManager {
  ...
  async register(input: {
    ...
    const sub: WebhookSubscription = {
      ...
      secret: input.secret ?? `whsec_${randomBytes(32).toString('hex')}`,
      createdAt: Date.now(),
    };
  ...
  async list(tenantId?: string): Promise<WebhookSubscription[]> {
    const subs = (await this.redis.hvals(SUBS_KEY)).map(
      (r) => JSON.parse(r) as WebhookSubscription,
    );
  ...
  async matching(tenantId: string, eventType: string): Promise<WebhookSubscription[]> {
    return (await this.list(tenantId)).filter((s) =>
      s.eventTypes.some((p) => globToRegex(p).test(eventType)),
    );
  }
}
