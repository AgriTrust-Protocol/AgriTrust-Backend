...
export function createAdminWebhooksRouter(
  ...
  router.get('/webhooks/deliveries', async (_req: Request, res: Response) =>
    res.status(200).json({ deliveries: await queue.list(100), depth: await queue.depth() }),
  );
  router.get('/webhooks/dead-letter', async (_req: Request, res: Response) =>
    res.status(200).json({ deadLetters: await deadLetters.list(100) }),
  );
  router.post('/webhooks/dead-letter/:id/replay', async (req: Request, res: Response) => {
    ...
    if (!dead) {
      res.status(404).json({ error: 'Dead-letter delivery not found' });
      return;
    }
  ...
  router.post('/webhooks/subscriptions', async (req: Request, res: Response) => {
    ...
    if (typeof tenantId !== 'string' || typeof url !== 'string') {
      res.status(400).json({ error: 'tenantId and url are required' });
      return;
    }
    res
      .status(201)
      .json({ subscription: await subscriptions.register({ tenantId, url, eventTypes, secret }) });
  });
  return router;
}
