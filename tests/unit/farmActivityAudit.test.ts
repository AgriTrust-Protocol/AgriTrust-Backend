...
import {
  canonicalJson,
  computeAuditHash,
  FarmActivityAuditService,
  AuditProof,
} from '../../src/audit/farmActivityAudit';
...
describe('farm activity audit hashing', () => {
  it('canonicalizes JSON before hashing so key order cannot alter proofs', () => {
    ...
    const one = computeAuditHash(prev, '11111111-1111-4111-8111-111111111111', timestamp, {
      b: 2,
      a: 1,
    });
    const two = computeAuditHash(prev, '11111111-1111-4111-8111-111111111111', timestamp, {
      a: 1,
      b: 2,
    });
  ...
  it('verifies chained audit proofs and rejects tampering', () => {
    ...
    const hashB = computeAuditHash(Buffer.from(hashA, 'hex'), eventB, timestampB, {
      action: 'treat',
    }).toString('hex');
      ...
      steps: [
        {
          ...
          payload: { action: 'plant' },
          prevHash: zero.toString('hex'),
          hash: hashA,
        },
        {
          eventId: eventB,
          timestamp: timestampB,
          payload: { action: 'treat' },
          prevHash: hashA,
          hash: hashB,
        },
      ],
    ...
    expect(
      service.verifyProof({
        ...proof,
        steps: proof.steps.map((step, i) =>
          i === 1 ? { ...step, payload: { action: 'harvest' } } : step,
        ),
      }),
    ).toBe(false);
  });
