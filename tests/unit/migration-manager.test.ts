...
async function makePool() {
  ...
  db.public.registerFunction({ name: 'pg_advisory_lock', args: [DataType.integer], returns: DataType.bool, implementation: () => true });
  db.public.registerFunction({ name: 'pg_advisory_unlock', args: [DataType.integer], returns: DataType.bool, implementation: () => true });
...
async function makeMigrationsDir(): Promise<string> {
  ...
  await writeFile(path.join(dir, '20260718000001_create_widgets.sql'), `
-- migrate:up
...
DROP TABLE widgets;
`);
  await writeFile(path.join(dir, '20260718000002_add_widget_status.sql'), `
-- migrate:up
...
describe('MigrationManager', () => {
  it('applies pending migrations in version order and records checksums', async () => {
    ...
    expect(result.executed.map((step) => step.version)).toEqual(['20260718000001', '20260718000002']);
    const rows = await pool.query('SELECT version, status, checksum FROM schema_migrations ORDER BY version');
  ...
  it('rolls migrations back to a target version in reverse order', async () => {
    ...
    expect(result.executed).toEqual([{ version: '20260718000002', name: 'add_widget_status', direction: 'down' }]);
    const status = await manager.status();
    expect(status.find((row) => row.version === '20260718000002')?.status).toBe('rolled_back');
    const columns = await pool.query("SELECT column_name FROM information_schema.columns WHERE table_name = 'widgets'");
  ...
  it('refuses rollback when a migration has no down section', async () => {
    const pool = await makePool();
    const dir = await mkdtemp(path.join(os.tmpdir(), 'agritrust-migrations-'));
    await writeFile(path.join(dir, '20260718000001_create_only_up.sql'), 'CREATE TABLE only_up (id TEXT PRIMARY KEY);');
